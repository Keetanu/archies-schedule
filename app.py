...
with st.sidebar:
    ...
    st.divider()
    stretch_bedtime = st.checkbox("🦒 Stretch Bedtime (7.5h Window)", value=False)
...
if lock or 'init' not in st.session_state:
    ...
    n_wake_s = clean_time(nw_s_in)
    n_wake_e = clean_time(nw_e_in)
    ...
    # WINDOW LOGIC: 6h first window, 7h/7.5h second window
    ...
    # Bedtime window based on toggle
    bed_window = 7.5 if stretch_bedtime else 7.0
    bedtime_dt = nap_end_dt + timedelta(hours=bed_window)
    ...
    c3.metric("Window 1 / 2", f"6h / {bed_window}h")
    ...
    sched = [
        ...
        (bedtime_dt - timedelta(hours=1), "🥛 Pre-Sleep Milk"),
        (bedtime_dt, f"✨ Bedtime ({bed_window}h Window)")
    ]
    ...
    with t2:
        ...
        pressures = []
        for t in times:
            if t < nap_start_dt: val = ((t - wake_dt).total_seconds()/3600)*16.6 # Hits 100 at 6h
            elif nap_start_dt <= t <= nap_end_dt: val = 15
            else: val = 20 + (((t - nap_end_dt).total_seconds()/3600)*(100/bed_window))
    ...
    with t3:
        try:
            ...
            if pr := st.chat_input("Ask the Guide..."):
                ...
                resp = model.generate_content(f"Archie 23mo. Sleep: {night_sleep}h. {pr}")
